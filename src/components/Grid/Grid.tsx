...
export interface IGridProps extends PropsWithChildren<any>{
  ...
  'flex-align-items' ?: 'center' | 'flex-start' | 'flex-end',
  'flex-direction' ?: 'column' | 'row' | 'column-reverse' | 'row-reverse'
  'flex-justify-content' ?: 'flex-start' | 'flex-end' | 'center' | 'space-around' | 'space-between'
}

const Grid = ({'flex-justify-content':flexJustifyContent,'flex-direction':flexDirection,'flex-align-items':alignItems,space,'space-lg':spaceLg,'space-md':spaceMd,'space-sm':spaceSm, 'size-lg':sizeLg, 'size-md': sizeMd,'size-sm':sizeSm,size,container,children} : IGridProps) => {

  const classRoot = React.useMemo(() => {
    const arr =  ['grid',`grid-${size}`,`flex-align-items-${alignItems}`,`flex-direction-${flexDirection}`,`flex-justify-content-${flexJustifyContent}`, `grid-space-${space}`]
