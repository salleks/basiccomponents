...
const InputText = (props : IInputTextProps) => {
  ...
  const {error, inputRef, icon, iconAction, fullWidth, label, required, helperText,align,  fontSize, ...rest} = props

  const rootClass = React.useMemo(() => {
    return `hw-input-text-root ${ error ? 'error' : '' } ${fontSize ? `${fontSize}` : ''}  ${ fullWidth ? 'full-width' : '' }  ${ props.readOnly ? 'readOnly' : '' }${align ? ` ${align}` : ' align-left'}  ${ props.disabled ? 'disabled' : '' }`
